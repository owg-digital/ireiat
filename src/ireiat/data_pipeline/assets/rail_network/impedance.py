...
def _generate_impedances(context: dagster.AssetExecutionContext,
                         g: ig.Graph,
                         separation_attribute="owner") -> set[tuple[str, str, str, str]]:
    """
                ...
                for in_edge_owner, out_edge_owner in product(in_owners, out_owners):
                    if in_edge_owner != out_edge_owner:
                        impedances.add((f"{in_edge_owner}{v.index}", f"{out_edge_owner}{v.index}",
                                        f"{in_edge_owner}", f"{out_edge_owner}"))
        else:
...
def _determine_impedance(from_rr: str, to_rr: str, impedance_rules: dict) -> float:
    """Use impedance rules to determine cost of using a railway interchange
    --Note:  the signature of this function will be expanded to include
             more information about the nodes in a later release.
             Keeping it simple for now"""

    # Hard coded.  Unlikely to change in the next 20 years
    class1_railways = {'BNSF', 'CN', 'CPKC', 'CSXT', 'NS', 'UP'}

    class1_to_class1_default_impedance = impedance_rules['class1_to_class1_default_impedance']
    class1_to_shortline_default_impedance = impedance_rules['class1_to_shortline_default_impedance']

    impedance = (class1_to_class1_default_impedance if from_rr in class1_railways and to_rr in class1_railways
                 else class1_to_shortline_default_impedance)
    return impedance


def generate_impedance_graph(context: dagster.AssetExecutionContext,
                             g: ig.Graph,
                             rail_interchange_impedances_src: dict,
                             separation_attribute="owner") -> ig.Graph:
    ...
    :param context: for logging output
    :param g: iGraph with `separation_attribute` of type `Set`
    :param rail_interchange_impedances_src: dict version of yaml input file for impedance rules
    :param separation_attribute: string identifying the attribute
    :return: an exploded graph with impedance edges

    """
    impedances = _generate_impedances(context, g, separation_attribute=separation_attribute)
    ...
    impedance_edges = [
        ((disjoint_union_vertex_mapping[from_id], disjoint_union_vertex_mapping[to_id]),
         _determine_impedance(from_rr, to_rr, rail_interchange_impedances_src))
        for from_id, to_id, from_rr, to_rr in impedances
    ]
    ...
    impedance_edge_attrs["impedance"] = [impedance for _, impedance in impedance_edges]
    disjoint_union.add_edges([edge for edge, _ in impedance_edges], impedance_edge_attrs)
    ...
    # Temporary code
    with open(r'c:\TEMP\impedances.pickle', 'wb') as impedance_file:
        pickle.dump([disjoint_union, impedances, impedance_edges, rail_interchange_impedances_src], impedance_file)
